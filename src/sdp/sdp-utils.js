...
function parseDtlsParameters(sdp) {
  const params = { role: 'auto', fingerprints: [] };
  for (const line of sdp.split(/\r?\n/)) {
...
function parseSctpParameters(sdp) {
  const params = { port: 5000, maxMessageSize: 262144 };
  for (const line of sdp.split(/\r?\n/)) {
 ...
 * @param {string} str
 */
function parseCandidateLine(str) {
